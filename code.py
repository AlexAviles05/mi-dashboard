...
if df_vacantes is not None and df_entrenamiento is not None:
    ...
    if 'plaza' in df_vacantes.columns and 'plaza' in df_entrenamiento.columns:
        ...
        # --- SECCIÓN 1: METRICS (CON INICIALIZACIÓN PREVENTIVA DE VARIABLES) ---
        st.subheader("📌 Resumen Ejecutivo General")
        col1, col2, col3, col4 = st.columns(4)
        
        # 🚨 FIX DE QA: Inicializar valores por defecto para evitar NameError si el set viene vacío
        ...
        if not df_vac_filtrado.empty:
            if dias_col and not df_vac_filtrado[dias_col].dropna().empty:
                dias_promedio = int(df_vac_filtrado[dias_col].mean())
            en_sies = len(df_vac_filtrado[df_vac_filtrado[estatus_col].astype(str).str.upper() == 'SIES'])
        
        # Calcular colaboradores únicos desde el concentrado
        ...
        # Renderizar tarjetas de métricas fijas de forma segura
        with col1:
            st.markdown(f'<div class="metric-box"><div class="metric-title">Total Vacantes Solicitadas</div><div class="metric-value">{total_vacantes}</div></div>', unsafe_allow_html=True)
        with col2:
            ...
            if f_ingreso and f_liberacion and zona_col:
                ...
                if not df_sla_clean.empty:
                    ...
                    fig_sla.add_hline(
                        ...
                        line_dash="dash", 
                        line_color="#e74c3c", 
                        annotation_text="SLA Objetivo (11 días)", 
